...
//Active or disable  video streaming from the drone. Needed in new firmware
Bebop.prototype.videoEnable = function(enable) {

  var buf = new Buffer(8);

  buf.writeUInt8(constants.ARCOMMANDS_ID_PROJECT_ARDRONE3, 0);
  buf.writeUInt8(constants.ARCOMMANDS_ID_ARDRONE3_CLASS_MEDIASTREAMING, 1);
  buf.writeUInt16LE(constants.ARCOMMANDS_ID_ARDRONE3_MEDIASTREAMING_CMD_VIDEOENABLE, 2);
  buf.writeUInt32LE(enable, 4);
...
Bebop.prototype.connect = function(callback) {
  this.discover(function() {
    ...
    //Send the date to the drone
    this.Common.currentDate(new Date().toISOString());
...
Bebop.prototype._packetReceiver = function(message) {
 var networkFrame = networkFrameParser(message);
  ...
  if (networkFrame.id === constants.BD_NET_DC_EVENT_ID ||
    ...
    try {
        //Get the correspondant event from the command file by project and class
      event = commands
        ...
        if (event.arg instanceof Array) {
          event.arg.forEach(function(arg) {
            if (types.hasOwnProperty(arg.type)) {
                ...
                {
                    console.log(err);
                    console.log("Command name: "+ arg.name +" arg type: "+ arg.type);
                }
            }
          });
        }
...
Bebop.prototype.stop = function() {
  this._pcmd = {
    flag: 0, //Tell the drone to stay stable
    roll: 0,
    pitch: 0,
    yaw: 0,
    gaz: 0,
